...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["usuario"]))){
        $usuario_err = "Please enter a password.";
    } elseif(strlen(trim($_POST["usuario"])) < 6){
        $usuario = "Password must have atleast 6 characters.";
    } else{
        $usuario = trim($_POST["password"]);
    }

    // Check input errors before inserting in database
    if(empty($usuario_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err)){
